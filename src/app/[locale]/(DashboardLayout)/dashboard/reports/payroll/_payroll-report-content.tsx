...
/**
 * Component hiển thị báo cáo lương chi tiết
 */
export function PayrollReportContent() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const data = await reportApi.getPayrollReport(filters);
      const chart = await reportApi.getPayrollChartData(filters);
  ...
  const handleExportCsv = async () => {
    ...
    setIsExporting(true);
    try {
      const blob = await exportCsv("payroll", getFilters());
  ...
  const handleExportPdf = async () => {
    ...
    setIsExporting(true);
    try {
      const blob = await exportPdf("payroll", getFilters());
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `payroll-report-${new Date().toISOString().split("T")[0]}.pdf`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const formatCellValue = (value: string | number, header: string): string => {
    if (typeof value === "number") {
      ...
      if (
        ...
        header.toLowerCase().includes("allowances") ||
    ...
    if (header.toLowerCase() === "status") {
      return getEnumLabel("payrollStatus", value, tEnums);
    }
    return String(value);
  };
              ...
              <Select value={status} onValueChange={setStatus}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="DRAFT">
                    {getEnumLabel("payrollStatus", "DRAFT", tEnums)}
                  </SelectItem>
                  <SelectItem value="FINALIZED">
                    {getEnumLabel("payrollStatus", "FINALIZED", tEnums)}
                  </SelectItem>
                  <SelectItem value="PAID">
                    {getEnumLabel("payrollStatus", "PAID", tEnums)}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
                      ...
                      <TableHeader>
                        <TableRow>
                          ...
                          {reportData.headers.map((header, index) => {
                            const headerKey = `payroll.column${header.charAt(0).toUpperCase() + header.slice(1)}`;
                            return (
                              <TableHead key={index}>
