 ...
 */
export function formatCurrencyWithSign(amount: number, locale: string): string {
  const formatted = formatPayslip(Math.abs(amount), locale);
...
/**
 * Component hiển thị số tiền theo locale của company
...
export function CurrencyDisplay({
  ...
  const { user } = useAuth();
  const companyLocale = user?.locale || "ja";
  ...
  if (showSign) {
    displayAmount = formatCurrencyWithSign(amount, companyLocale);
  } else {
    displayAmount = formatPayslip(amount, companyLocale);
  }
...
export function SalaryDisplay({
  ...
  const displayAmount = formatPayslip(amount, companyLocale);
...
export function DeductionDisplay({
  ...
  const displayAmount = `-${formatPayslip(Math.abs(amount), companyLocale)}`;
...
export function BalanceDisplay({
  ...
  const displayBalance = formatPayslip(balance, companyLocale);
