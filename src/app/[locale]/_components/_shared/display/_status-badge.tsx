...
export function getPayrollPeriodStatusVariant(
  status: PayrollPeriodStatus,
): BadgeVariant {
  return PAYROLL_PERIOD_STATUS_COLORS[status] || "default";
}
...
export function PayrollPeriodStatusBadge({
  status,
  className,
}: PayrollPeriodStatusBadgeProps) {
  const tEnums = useTranslations("enums");
  const variant = getPayrollPeriodStatusVariant(status);
  const label = tEnums(`payrollPeriodStatus.${status}`);
