...
export function PayslipCard({
  ...
  const getItemName = (code: string, name: string) => {
    if (SYSTEM_ITEM_CODES.includes(code)) {
      return tPayroll(`systemItems.${code}`);
    }
    return name;
  };
    ...
    <GlassCard className="w-full p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:justify-between gap-4 mb-6">
        <div className="flex-1">
          <h2 className="text-lg md:text-xl font-bold mb-2">{formatTitle()}</h2>
          <p className="text-sm text-muted-foreground mb-4">{formatPeriod()}</p>
          <div className="flex items-baseline gap-2">
            <span className="text-sm font-medium">{t("netSalary")}:</span>
            <span className="text-xl md:text-2xl font-bold">
              {formatPayslip(payslip.netSalary, companyLocale)}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <PayslipColumn title={t("earnings")}>
          <PayslipRow
            label={t("baseSalary")}
            value={formatPayslip(payslip.baseSalary, companyLocale)}
          />
          {payslip.allowanceDetails?.map((item, index) => (
            <PayslipRow
              key={index}
              label={getItemName(item.code, item.name) || t("allowance")}
              value={formatPayslip(item.amount, companyLocale)}
            />
          ))}
          {payslip.totalOvertimePay > 0 && (
            <PayslipRow
              label={t("overtimePay")}
              value={formatPayslip(payslip.totalOvertimePay, companyLocale)}
            />
          )}
          <PayslipTotalRow
            label={t("total")}
            value={formatPayslip(payslip.grossSalary, companyLocale)}
          />
        </PayslipColumn>
        ...
        <PayslipColumn title={t("deductions")}>
          {payslip.deductionDetails?.map((item, index) => (
            <PayslipRow
              key={index}
              label={getItemName(item.code, item.name) || t("deductions")}
              value={formatPayslip(item.amount, companyLocale)}
            />
          ))}
          ...
          <PayslipTotalRow
            label={t("total")}
            value={formatPayslip(payslip.totalDeductions, companyLocale)}
          />
        </PayslipColumn>
        ...
        <PayslipColumn title={t("others")}>
          <PayslipRow
            ...
            value={formatPayslip(payslip.netSalary, companyLocale)}
          />
        </PayslipColumn>
      </div>
